...
| Solution blueprint (LEGACY) | `discover-external-sources`, `apply-blueprint-answer`, `apply-uc-intake`. `prepare-solution-blueprint` is retired -- it redirects to `prepare-blueprint` (Task D1). `apply-blueprint-answer` still edits artifacts produced before that redirect. |
...
| Provisioning + ingestion | `plan-provisioning`, `apply-provisioning`, `generate-ingestion`, `sync-workspace-code` -- additive-only Unity Catalog provisioning, generated Databricks-native ingestion code, and shipping that generated code to the workspace via `databricks sync` (never `workspace import-dir`, which strips file extensions) |
