...
| Cloud-native (dbt/Airflow) | `check-platform-readiness`, `generate-dbt-project`, `prepare-data-quality-panel`, `apply-data-quality-answer`, `verify-dbt-project`, `verify-orchestration`, `dbt-index`, `run-dbt-backfill`, `reconcile-warehouse-cost`, `publish-dbt-state` (ship `target/manifest.json`+`run_results.json` to a UC volume after a run -- unlocks slim CI, `dbt retry`, `dbt clone`), `check-airflow-health` (scheduler reachable + DAG not paused; a paused DAG is `ok: false`, never silently healthy) |
...
| Solution blueprint (LEGACY) | `discover-external-sources`, `prepare-solution-blueprint` (RETIRED -- redirects to `prepare-blueprint`, Task D1; writes nothing of its own), `apply-blueprint-answer` (still edits artifacts produced before that redirect), `apply-uc-intake` |
...
| Provisioning + ingestion | `plan-provisioning`, `apply-provisioning`, `generate-ingestion`, `run-ingestion` (executes the emitted `COPY INTO`/Auto Loader jobs against the warehouse -- refusal ladder: unconfirmed blueprint means dry run, kill switch means refuse, unreachable means a structured failure pointing at `check-platform-readiness`; stops on the first failure and tags the rest `not_attempted`), `sync-workspace-code` -- additive-only Unity Catalog provisioning, generated Databricks-native ingestion code, and shipping that generated code to the workspace via `databricks sync` (never `workspace import-dir`, which strips file extensions) |
