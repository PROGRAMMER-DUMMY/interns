"""``workspace-dashboard-deck``: export a workspace's live MinusAnalyst dashboard
to an interactive PowerPoint (the MinusDeck vendor).

Reads the same certified conformed model the live dashboard serves, then renders
an interactive .pptx -- a clickable contents slide, a section slide per page
(KPI scorecard + chart thumbnails), and a detail slide per chart/table, wired
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        ...
        description="Interactive PPTX export (MinusDeck) of the live dashboard.")
    ap.add_argument("--workspace", required=True)
    ap.add_argument("--repo-root", default=str(PROJECT_ROOT))
    ap.add_argument("--out", default=None, help="Output .pptx (default dashboard/exports/<name>.pptx).")
    ap.add_argument("--theme", default=None, help="Override theme (claude / dark).")
    ap.add_argument("--charts", choices=["native", "image"], default="native",
                    help="native = editable Excel-backed PowerPoint charts (default); "
    ...
    ap.add_argument("--open", dest="open_after", action="store_true",
                    help="Open the .pptx when done.")
    ap.add_argument("--screen", action="store_true",
                    help="Build, rasterize every slide, run deterministic checks, and "
    ...
    if args.screen:
        ...
        summary = screen_exports(repo_root, args.workspace, kinds=("pptx",),
                                 charts=args.charts, theme=args.theme, force=args.force)
        if args.json:
    ...
    out = Path(args.out) if args.out else default_out(workspace, "pptx")
    add_vendor_to_path(repo_root)
    from minus_deck import build_deck

    res = build_deck(root, out, theme=args.theme, charts=args.charts)
    for w in warnings:
        print(f"[~] {w}", file=sys.stderr)
    if args.json:
        print(json.dumps({"action": "pptx", **res, "warnings": warnings}, indent=2))
    else:
        print(f"[ok] MinusDeck PPTX -> {res['path']}")
        print(f"     {res['slides']} slides ({res['detail_slides']} detail), "
              f"{res['pages']} pages, theme {res['theme']}")
        print(f"     charts: {res['native_charts']} native (double-click to Edit Data), "
