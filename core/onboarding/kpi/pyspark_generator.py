        ...
        *,
        ...
        if self.dialect == "databricks":
            ...
            lines += [
                ...
                "# Scratch dir for shuffle/blockmgr files: an explicit dir (from the",
                "# verifier, or a self-owned mkdtemp when run standalone) instead of the",
                "# OS temp root default, so a killed/timed-out run leaves a single",
                "# identifiable directory behind rather than untracked files in %TEMP%.",
                "import tempfile as _tempfile",
                '_spark_local_dir = os.environ.get("SPARK_LOCAL_DIR_OVERRIDE") or _tempfile.mkdtemp(prefix="spark_local_")',
                "_builder = (",
                "    SparkSession.builder",
                f'    .appName("{self.app_name}_{kpi_id}")',
                '    .config("spark.local.dir", _spark_local_dir)',
