...
- Function [`contextual_column_candidates(feature, full_context, schema_index, *, metric="")`](file:///C:/Users/shubh/OneDrive/Desktop/interns/core/onboarding/kpi/feature_resolver.py#L1184-L1262) - *Score columns against a feature using the surrounding KPI context.*
  - **The numeric-aggregate disqualifier.** `metric` drives one hard exclusion: a feature the KPI sums or averages cannot be a column whose OBSERVED evidence says it is not numeric. Found live -- `sum(ContractedRate)` was offered `payors.PayorName`, a String of company names, because "payor" overlapped both the KPI text (+2 column tokens) and the dataset name (+6), clearing the score>=8 threshold with nothing to subtract. It reached the panel as a selectable option, so a human could accept it and the provenance would record a human decision on a wrong number.
  - It is a **disqualifier, not a negative bonus**, because in `_contextual_score` name signals reach **+30** (table name == feature) while every value-evidence rule caps at +4 -- any penalty small enough to be safe is also small enough to be outvoted. The only other penalty in that function is likewise a hard `-30` (key/ID column), so this follows existing precedent.
  - Keyed on the AGGREGATE (`_numerically_aggregated`), never the feature name: `ContractedRate` used as a cut keeps its text candidates; only `sum(...)`/`avg(...)` over it excludes them. Absent evidence never disqualifies (`_contradicts_numeric_aggregate` returns False when dtype AND value_pattern are both empty) -- unknown must stay non-accusing, or a weaker profiler would silently drop real candidates.
- `_derivation_pattern_options` still swallows a detector crash (pattern detection is advisory and must not break resolution) but records the reason in `self.derivation_pattern_error`. The previous bare `except` made a broken detector indistinguishable from "no pattern applies" -- the same failure class as F25, and its `pragma: no cover` guaranteed no test would ever see it.
