...
def _core_tree_fingerprint() -> str:
    """Stable hash of the platform's own source tree (path + size + mtime).

    Platform code must not change while a workspace pipeline is running: a
    generator edited mid-run invalidates every artifact produced before it. The
    2026-07-26 audit found seven core/ files edited between pipeline dispatches
    -- including a deleted fan-trap detector -- with the pipeline reporting no
    opinion about it.

    Cheap on purpose: stat only, no file reads.
    """
    digest = hashlib.sha256()
    for base in ("core", "tools"):
...
def pipeline_main(argv: list[str] | None = None) -> int:
    ...
    args = parser.parse_args(argv)
    core_fingerprint_at_start = _core_tree_fingerprint()
    repo_root = Path(args.repo_root).resolve()
    ...
    if result.status == "complete":
        ...
        if _core_tree_fingerprint() != core_fingerprint_at_start:
            print("[blocked] core_drift: platform source under core/ or tools/ changed")
            print("          during this run, so artifacts produced earlier were")
            print("          generated by different code than what is on disk now.")
            print("")
            print("Re-run from a clean tree:")
            print(f"  run-kpi-pipeline --workspace {workspace_rel} --domain {domain} --new-session")
            return 2
