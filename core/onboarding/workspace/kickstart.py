...
class WorkspaceKickstarter:
    ...
    def run(self) -> KickstartResult:
        ...
        bug_detector = WorkspaceBugDetector(self.repo_root, _rel(self.workspace, self.repo_root))
        bug_report = bug_detector.run()
        if bug_report.blocks_workflow:
            artifacts = bug_detector.write_report(bug_report)
            raise RuntimeError(
                "blocking workspace bugs detected; review "
                f"{artifacts['markdown']} before kickstart can continue"
            )
