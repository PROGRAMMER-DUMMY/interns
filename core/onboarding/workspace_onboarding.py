...
class WorkspaceOnboarder:
    ...
    def run(self) -> OnboardingResult:
        ...
        profiles, profile_warnings = self.profile_inputs(inputs.data_files)
    ...
    def profile_inputs(self, data_files: list[str]) -> tuple[list[dict[str, Any]], list[str]]:
...
def _read_tabular_kpis(frame: Any, source: str) -> list[KpiDefinition]:
    ...
    cuts_col = _first_existing(lowered, ["cuts / grain hints", "cuts", "grain", "dimensions"])
...
def _read_xlsx_xml_kpis(path: Path) -> list[KpiDefinition]:
    ...
    cuts_idx = _first_index(index, ["cuts / grain hints", "cuts", "grain", "dimensions"])
...
def _read_json_kpis(path: Path, repo_root: Path) -> list[KpiDefinition]:
    ...
    for item in entries:
        if not isinstance(item, dict):
            continue
        name = _clean_cell(item.get("name") or item.get("kpi") or item.get("question"))
        if name:
            kpis.append(
                KpiDefinition(
                    name=name,
