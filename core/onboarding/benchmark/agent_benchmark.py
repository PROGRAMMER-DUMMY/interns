...
class AgentBenchmarkScorecardBuilder:
    ...
    def _components(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "kpi_execution_harness": self._kpi_execution_harness_component(),
    ...
    def _data_model_readiness_component(self) -> dict[str, Any]:
        ...
        domain_path = self.layout.contracts_dir / "domain_model.json"
        path = final_path if final_path.exists() else draft_path if draft_path.exists() else domain_path
        ...
        score = float((readiness.get("overall") or {}).get("score") or (90 if path in {final_path, domain_path} else 55))
        blockers = []
        ...
        return _component(
            ...
            status=(
                "ready"
                if score >= 80 and path in {final_path, domain_path}
                else "needs_review"
                if score >= 60
                else "blocked"
            ),
            ...
            blockers=blockers,
            details={
                "state": "finalized" if path == final_path else "onboarded_domain_model" if path == domain_path else "draft",
                "table_count": len(model.get("tables", []) or model.get("datasets", [])),
                "relationship_count": len(model.get("relationships", []) or model.get("data_models", [])),
            },
        )
    ...
    def _kpi_execution_harness_component(self) -> dict[str, Any]:
        ...
        if not payload:
            return _component(
                ...
                evidence=evidence,
                blockers=[
                    "KPI execution harness evidence is missing; run the KPI execution harness before executable SQL generation."
                ],
                details={"state": "missing", "artifact": "kpi_execution_harness.json"},
            )

        blockers: list[str] = []
        if payload.get("artifact_type") != "kpi_execution_harness.json":
            blockers.append("kpi_execution_harness.json missing `artifact_type: kpi_execution_harness.json`")
        if payload.get("version") != 1:
            blockers.append("kpi_execution_harness.json has unsupported or missing `version`")
        if payload.get("generated_by") != "run-kpi-execution-harness":
            blockers.append("kpi_execution_harness.json must be generated by run-kpi-execution-harness")
        if payload.get("ok") is not True:
            blockers.append("KPI execution harness did not pass")

        records = payload.get("records")
        if not isinstance(records, list) or not records:
            blockers.append("KPI execution harness must include non-empty `records`")
            records = []

        passed_count = 0
        failed_count = 0
        exact_result_view_count = 0
        sample_table_count = 0
        for idx, record in enumerate(records, start=1):
            if not isinstance(record, dict):
                blockers.append(f"harness record #{idx} is not a JSON object")
                failed_count += 1
                continue
            ...
            result_view = str(record.get("result_view") or "")
            sample_output_table = record.get("sample_output_table")
            columns = record.get("columns")
            record_ok = True

            if status != "passed":
                blockers.append(f"harness record #{idx} for `{kpi_id}` did not pass")
                record_ok = False
            if result_view != expected_view:
                blockers.append(
                    f"harness record #{idx} does not target exact result view `{expected_view}`"
                )
                record_ok = False
            if not isinstance(columns, list) or not columns:
                blockers.append(f"harness record #{idx} for `{kpi_id}` has no result columns")
                record_ok = False
            elif _placeholder_result_columns(columns):
                blockers.append(f"harness record #{idx} for `{kpi_id}` has only placeholder result columns")
                record_ok = False
            if not isinstance(sample_output_table, str) or "|" not in sample_output_table:
                blockers.append(
                    f"harness record #{idx} for `{kpi_id}` missing table-form sample output"
                )
                record_ok = False

            if record_ok:
                passed_count += 1
                exact_result_view_count += 1
                sample_table_count += 1
            else:
                failed_count += 1

        status = "ready" if payload.get("ok") is True and not blockers and records else "blocked"
        score = 100.0 if status == "ready" else 0.0 if payload.get("ok") is not True else 35.0
        return _component(
            score=score,
            status=status,
            evidence=evidence,
            blockers=blockers,
            details={
                "state": "passed" if status == "ready" else "failed",
                "artifact": "kpi_execution_harness.json",
                "ok": bool(payload.get("ok")),
                "kpi_count": len(records),
                "passed_count": passed_count,
                "failed_count": failed_count,
                "exact_result_view_count": exact_result_view_count,
    ...
    def _release_gates(
        ...
        harness_ok = components["kpi_execution_harness"]["status"] == "ready"
        ...
        return [
            ...
            _gate("executable_sql_generation", kpi_ok and model_ok and rel_ok and stt_ok and validation_ok and harness_ok, "Executable SQL generation requires KPI, model, relationship, source-to-target, validation, and execution harness proof.", ["build-relationship-contracts", "plan-source-to-target"]),
            ...
            _gate("production_promotion", core_readiness >= 85 and validation_ok and rel_ok and stt_ok and harness_ok, "Production promotion needs high core readiness and no executable blockers.", ["prepare-agent-benchmark"]),
        ]
...
def _blocker_routes(
    ...
    if components["kpi_execution_harness"]["status"] != "ready":
        routes.append(
            {
                "blocker_type": "kpi_execution_harness",
                "next_command": f"uv run python -m core.onboarding.kpi.execution_harness --workspace {workspace}",
                "reason": "Run the KPI execution harness and verify final result views before executable SQL generation.",
            }
        )
