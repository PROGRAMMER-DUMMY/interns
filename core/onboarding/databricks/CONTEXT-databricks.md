...
- **Key Functions / Classes**:
  ...
  - [`check_local_green(workspace)`](file:///C:/Users/shubh/OneDrive/Desktop/interns/core/onboarding/databricks/deploy_gates.py#L61-L105): Gate G1: Checks latest medallion run (`run.json`) for failed tables, degraded runs, unequal KPI diffs, and checks `kpi_execution_harness.json`. An unlistable runs directory (`OSError`/`PermissionError`) returns a failing verdict naming the path — it never propagates, because a gate that raises has blocked nothing.
  - [`check_design_ratified(workspace)`](file:///C:/Users/shubh/OneDrive/Desktop/interns/core/onboarding/databricks/deploy_gates.py#L107-L140): Gate G2: Ensures zero open items in `medallion_design_panel/current.json`. A non-numeric `open_count`, or an `items` entry that is not a dict, fails closed with a named reason rather than raising.
...
- ⚠️ **Broken References & Mismatches**:
  - In `deploy_gates.py` ([`check_plan_freshness`](file:///C:/Users/shubh/OneDrive/Desktop/interns/core/onboarding/databricks/deploy_gates.py#L168-L173)), `_load_manifest` is imported dynamically inside function scope from `core.medallion.deploy_plan`. This is NOT a fragility: the import sits inside the `try`, and `except Exception` returns a failing `GateVerdict` naming the failure. A missing or refactored `core.medallion.deploy_plan` blocks the deploy cleanly. (An earlier revision of this doc claimed the opposite; corrected 2026-08-09.)
