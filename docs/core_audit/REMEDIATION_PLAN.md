...
| P2 gates | fix/core-p2-gates | (pending) | done (code) | tests/regressions/test_core_p2_gates.py | Genie lane enforces G3 human-provenance (--confirmed-by, records all verdicts); remote-target gate -> local allow-list fail-closed; bootstrap health_check gated behind remote-approval env (no network on local-safe path); external-root allowlist (is_within_allowed_roots) on discovery + local-source; SSRF egress guard on api fetch. 2 sub-commits (P2a/b) pushed. No net-new failures. NOTE: parallel background agents (worktree-isolated AND non-isolated) both structurally blocked in this env (wrong base / sandbox), so done sequentially. |
...
| P4 concurrency | fix/core-p4-concurrency | (pending) | done (code) | tests/regressions/test_core_p4_concurrency.py (9) | new core/storage/atomic_io.py (write_*_atomic, read_json_or_quarantine fail-loud, thread-safe pushd). P4a: memory stores atomic + corrupt-store quarantine+raise (no silent erase). P4b: os.chdir removed from 6 modules -> pushd (no bare os.chdir in lib). P4c: workspace.py SQLite WAL + busy_timeout + check_same_thread=False + RLock-guarded writes. P4d: sources allowlist lock+re-read+atomic, confirm-cli atomic writes. 4 sub-commits pushed. DEFERRED (lower-freq/higher-risk): confirm-cli lock, Delta metadata append->merge upsert, POSIX workspace_lock unlink/reclaim race. |
| P5 correctness | fix/core-p5-correctness | (pending) | partial (code) | tests/regressions/test_core_p5_correctness.py (8) | DONE: P5a Databricks success-enum (poll_job_run bare name + tolerant check) + optimization direction default; P5b substring->token (result_view_builder exact-alias dedupe, base_source_selector word-token grain match); P5c parity coverage (engine_recommender never routes pyspark/hybrid -> certified polars; _normalize_cell keeps ints exact + integral-float canonical); P5d un-gated dedup (silver SELECT * not DISTINCT). 4 sub-commits pushed. DEFERRED (need careful per-site analysis, own session): remaining T9 sites (features 2-char hints, intent_coverage LIMIT-5, external_discovery .jsonl precedence); relationships/contracts free-text join gate (clears test_kpi_pipeline_wrapper baseline); medallion build.py no-ops (652/792) + design.py:884; wiki _split_sections. |
| P6 silent-except | fix/core-p6-silent-except | (pending) | done (code) | tests/regressions/test_core_p6_silent_except.py (4) | data_quality: read/parse failure now a `dataset_unreadable` finding (gate can't report ok=True on unreadable data); narrowed 3 excepts. documents/candidate_apply: corrupt accepted_candidates fails loud (quarantine+raise) instead of skeleton-overwrite that erased decisions. sources/catalog._read_existing_rows except narrowed. Broader `except Exception` sweep across core/ left as a follow-up (triage, lower-risk). |
| P7 wiring | fix/core-p7-wiring | (pending) | partial (code) | (updated test_op_signals; uses existing reliability/guard suites) | DONE: op_signals stuck-normalize + stale-skill-name fix; reliability_suite now blocks on failed/malformed/missing-required harness artifacts (layered/pipeline-exec/data-quality); workflow_guard flags raw dataset paths in generated SQL outside CATALOG BOOTSTRAP. Full gate 23->14 failures. DEFERRED: workflow_guard external-profile + command-log/time-budget checks (3 tests); agents engine routing (honor main_agent -> CLI-agent LLM); governor/runner wire-or-delete; context doc_retrieval; data_model promise reconcile; flow full_kpi_sql vs usual_workflow intents. |
| P8 cleanup | fix/core-p8-cleanup | (pending) | partial (code) | test_source_family_contracts + genericity audit | DONE (T12 leaks): removed hardcoded healthcare procedure->description/code branch in feature_resolver:1219 (generic dictionary-overlap rules cover it); source_family_contracts partition_columns now DERIVED from schema (year/period-like columns present) not hardcoded ["report_year"] + test updated to evidence-based contract. DEFERRED: dead code (contracts/versioning.migrate, medallion budget/tier_router, dashboard non-SQL path, StubLLMEngine, post_api); dup helpers (_rel/_norm/_slug/_now consolidation); medallion_design_naming heuristics (2 baseline tests — ambiguous naming semantics, need careful design); flow.py decomposition (deferred per user). |
